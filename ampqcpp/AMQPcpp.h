...
class AMQPException
{
	...
	int code;
	public:
		AMQPException(const string & message);
		AMQPException(const string & message, int errcode);
...
class AMQPMessage {
	...
	public :
		...
		void setMessage(const char * data,uint32_t length, bool copy = true);
		int addFragment(const char * data, uint32_t offset, uint32_t length);
...
class AMQPQueue : protected AMQPBase 
{
	public:
		class EventListener
		{
		public:
			EventListener() { q = NULL; }
			virtual int onMessage( AMQPMessage *m) = 0;
			virtual void onCancel( AMQPMessage *m );
			AMQPQueue * q;
			virtual ~EventListener();
		};
	protected:
		// queues must be created using an AMPQ connection object
		...
		int prepareBuffer(size_t len);
		int processBodyFrag(void *data, size_t len);
	public:
		string getName() { return name; }
		void Declare(short parms = AMQP_AUTODELETE);
		...
		void addEvent( EventListener * listner );
...
class AMQPExchange : public AMQPBase
{
	...
	public:
		...
		void Declare(short parms = 0);
		/**
		 * Declare: declare this exchange on the Rabbi MQ bus
		 *          exchange name is the one set when echange object
		 *	    was created by AMQP::createExchange(name)
		 *
		 * @param exchtype: type of exchange. Valid values are
		 *         direct, topic, fanout.
		 **/
		void Declare(const string & exchtype, short parms = 0);
		...
		void setHeader(string name, string value, bool special = false);
...
class AMQP
{
	...
	public:
		...
		int runChannels(int nbMessageToProcess);
	...
	private:
		...
		void sockConnect();
		void login();
		int processMethodFrame(AMQPQueue *q, amqp_frame_t frame);
};

/** 
 * bytes to string (utility)
 */

inline string AmqpbytesToString(amqp_bytes_t data)
{
    return string( (const char*) data.bytes, data.len );
}
#endif //__AMQPCPP
