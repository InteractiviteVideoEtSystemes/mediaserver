...
AMQPQueue::~AMQPQueue()
{
	...
	cout << " desttroying queue " << name << " on channel " << getChannelNum() << endl;
	// Do auto cleanum
	if ( parms & AMQP_AUTODELETE )
	{
		cout << " autoclean: deleting queue on bus"  << endl;
		Delete();
	}
	this->closeChannel();
	if (pmessage)
...
void AMQPQueue::Declare(short parms)
{
	this->parms=parms;
	sendDeclareCommand();
}
...
{
	/* IVeS support for private queue */
	//if (!name.size())
	...
	*/
	...
	if (res.reply.id == AMQP_CHANNEL_CLOSE_METHOD || res.reply.id == AMQP_CONNECTION_CLOSE_METHOD)
	{
		...
		throw AMQPException(&res);
			...
			// Get name allocated by rabbit mq
			if (name.size() == 0)
			{
			    name = AmqpbytesToString( data->queue );
			}
	} else {
...
void AMQPQueue::addEvent( EventListener * listener)
{
	if ( listener != NULL )
	{
		listener->q = this;
...
void AMQPQueue::sendConsumeCommand() {
	...
	*/
	...
	switch ( res.reply_type )
	{
		...
		throw AMQPException(error_message, err->reply_code);
		break;
	    }
	...
	while(1) {
		...
		if (frame.payload.method.id == AMQP_BASIC_CANCEL_OK_METHOD)
		{
			...
			cout << "CANCEL received on queue " << name << endl;
			if (listener)
			{
				listener->onCancel(pmessage);
			}
			amqp_maybe_release_buffers(*cnn);
			pmessage = NULL;
			break;
		...
		if (frame.payload.method.id == AMQP_CONNECTION_CLOSE_OK_METHOD)
		{
			cout << "consume: channel " << frame.channel << " closed." << endl;
			amqp_maybe_release_buffers(*cnn);
			pmessage = NULL;
			break;
		}
		...
		if (frame.payload.method.id != AMQP_BASIC_DELIVER_METHOD){
			cout << "NON BASIC_DELIVER_METHOD continue  method.id=" << \
				amqp_method_name(frame.payload.method.id) << " on queue " << name << endl;
			continue;
		}
		...
		//if (result <= 0) {
		if (result < 0) {	
			cout << "amqp_simple_wait_frame() returned " << result << ". err=" << strerror(errno) << endl;
			throw AMQPException("Failed to read frame.");
			return;
		}

		if (frame.frame_type != AMQP_FRAME_HEADER) {
			cout << " frame type = " << frame.frame_type << " result = " << result << endl;
		...
		*(buf+body_target)='\0';
		...
		if (listener)
		{
			int ret = listener->onMessage(pmessage);
			if (ret) break;
		}
	}
#endif
...
int AMQPQueue::prepareBuffer(size_t len)
{
    if (pmessage)
    {
	pmessage->setMessage(NULL, len, false);
	body_received = 0;
	return 0;
    }
    else
...
int AMQPQueue::processBodyFrag(void *data, size_t len)
{
    if (pmessage)
    {
	uint32_t bufLen;
	void *buf = pmessage->getMessage(&bufLen);

	if (bufLen == 0) return -2; // No buffer allocated

	int ret = pmessage->addFragment( (const char *) data, body_received, len);
	switch(ret)
	{
	    case 0:
		body_received +=len;
		return 0; // message incmplete
		...
		if ( listener )
		{
		    if (listener->q == NULL) listener->q = this;
		    ret = listener->onMessage(pmessage);
		}
		if (pmessage)
		{
		    delete pmessage;
    ...
    return -1; // no pmessage: body unexpected
}
	
void AMQPQueue::EventListener::onCancel( AMQPMessage *m )
{
}

AMQPQueue::EventListener::~EventListener()
{
}
