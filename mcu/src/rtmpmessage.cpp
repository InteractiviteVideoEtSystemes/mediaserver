...
RTMPMessage::RTMPMessage(DWORD streamId,QWORD timestamp,Type type,DWORD length) 
{
	...
	switch(type)
	{
		...
		case CommandAMF3:
			...
			skip = true;
			//Create cmd message
			Log("RTMPMessage: warning, AMF3 command.\n");
		...
		case DataAMF3:
			...
			skip = true;
			//Reset amf parser
			Log("RTMPMessage: warning, AMF3 meta data.\n");
...
DWORD RTMPMessage::Parse(BYTE* data,DWORD size)
{
	...
	if (IsControlProtocolMessage())
	{
		...
		//Check if we have finished
		parsing = (pos!=length);
		if (len == 0) Log("RTMPChunk: empty media frame.\n");
	} else if (IsMetaData()) {
		if (meta)
		{
			//If it is an amf3 object skip the first byte if 0
        ...
        {
            Error("RTMPParser: unsupported message of type %d of size %u. Returning, len=%u.\n",
                type, size, len);
            ::Dump(data, size);
	    std::range_error* e = new std::range_error("Unsupported RTMP message type");
	    throw *e;
        }
	return len;
}
