...
AMQPException::AMQPException(const string &message) {
	this->message= message;
}

AMQPException::AMQPException(const string &message, int errCode)
{
     this->message = message;
     if ( errCode == 0 ) this->message += ": no error";
     else if (errCode < 0)
     {
	   // socket error are returned as negative value
	   char * msg = amqp_error_string( -errCode );
	   if (msg)
	   {
	       this->message += ": ";
	       this->message += msg;
     ...
     {
	   // when errcode is psotive (normal regular case)
	   char * msg = amqp_error_string( errCode );
	   if (msg)
	   {
	       this->message += ": ";
