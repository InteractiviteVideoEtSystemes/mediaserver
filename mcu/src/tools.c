 ...
 **************************************/
void calcAbsTimeout(struct timespec *ts, const struct timeval *tp,DWORD timeout)
{
...
long compareTime(const struct timespec *ts1, const struct timeval *ts2)
{
	int diff;
	struct timeval tts1, *tts2, now;

	gettimeofday(&now,0);
	if (ts1)
	{
		tts1.tv_sec= ts1->tv_sec;
		tts1.tv_usec = ts1->tv_nsec/1000;
	}
	else
	{
		tts1 = now;
	}
	tts2 = (ts2 != NULL) ? ts2 : &now;


	return (tts1.tv_sec - tts2->tv_sec)*1000 + ( tts1.tv_usec - tts2->tv_usec ) / 1000;
}
