...
void AMQPExchange::Declare(short parms)
{
	string extype("direct");
	Declare(extype, parms);
}

void AMQPExchange::Declare(const string & extype, short parms)
{
	this->type = extype;
	this->parms=parms;
	sendDeclareCommand();
...
{
	amqp_bytes_t exchangeByte;
	//if ( name.size() > 0 )
	//	exchangeByte = amqp_cstring_bytes(name.c_str());
	//else
		exchangeByte = amqp_cstring_bytes("");
