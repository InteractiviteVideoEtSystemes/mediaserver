...
RTMPMediaStream::RTMPMediaStream()
{
	this->id = 0;
	this->data = 0;
        gettimeofday(&lastCongestionTime, NULL);
}
...
bool RTMPMediaStream::SendMediaFrame(RTMPMediaFrame* frame)
{
	bool ret = true;
        timeval beginMesure;
	...
	{
		//Send it	
		if ( (*it)->onMediaFrame(id,frame) == false )
			ret = false;
	}
	//Unlock
	lock.DecUse();
        QWORD elapsed = getDifTime(&beginMesure)/1000;
        if ( elapsed > 20 ) Error("-RTMPMediaStream: SendMediaFrame took %lu ms.\n",elapsed);
	return ret;
}
...
void RTMPMediaStream::onCongestion()
{
    if (! congested)
    {
        // Max every 2 seconds
        if (getDifTime(&lastCongestionTime)/1000 > 2000)
        {
            congested = true;
            gettimeofday(&lastCongestionTime, NULL);
        }
...
bool RTMPPipedMediaStream:: onMediaFrame(DWORD id,RTMPMediaFrame *frame)
{
	//Get timestamp
	bool ret = true;
...
bool RTMPCachedPipedMediaStream::SendMediaFrame(RTMPMediaFrame *frame)
{
