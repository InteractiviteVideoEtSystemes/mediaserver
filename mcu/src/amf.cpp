...
DWORD AMFParser::Parse(BYTE *data,DWORD size)
{
	...
	switch((TypeMarker)data[0])
	{
		...
		default:
			//Create the object
			Error("AMFParser: unknown marker %02x.\n", data[0]);
			AMFData::DumpCallStack();
...
{ 
	if(GetType()!=type)
	{
		// Detailed trace to troobleshoot things
		const char *typeStr1, *typeStr2;
		char msg[200];
		
		typeStr1 = TypeToString( GetType() );
	        typeStr2 = TypeToString( type );
		
	     
		Error("AMF: Incorrect type. Expected %s and parsed %s. Call stack follows\n",  typeStr2, typeStr1);
		DumpCallStack();

		sprintf(msg, "AMFData cast exception. Expected %s, provided %s",  typeStr2, typeStr1);
