...
init:
	...
	{
		//Log
		if ( popingfd > 0 ) Log("-RTMP Server accepting connections [fd:%d]\n", ufds[0].fd);

		//Clean zombies each time
		CleanZombies();

		popingfd = poll(ufds,1,10000);
		//Wait for events
		if (popingfd<0)
		{
			//Error
...
void RTMPServer::onDisconnect(RTMPConnection *con)
{
	...
	{
		Log("-RTMP onDisconnected [%p]: connection was not in the active list.\n",con);
	}
