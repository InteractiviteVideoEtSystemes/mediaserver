...
bool RTMPClientConnection::onMediaFrame(DWORD streamId,RTMPMediaFrame *frame)
{
	bool ret;
	...
	switch(frame->GetType())
	{
		case RTMPMediaFrame::Audio:
			...
			ret = chunkOutputStreams[4]->SendMessage(new RTMPMessage(streamId,ts,frame->Clone()));
			break;	
		case RTMPMediaFrame::Video:
			ret = chunkOutputStreams[5]->SendMessage(new RTMPMessage(streamId,ts,frame->Clone()));
			break;
	}
	//Signal frames
	SignalWriteNeeded();
