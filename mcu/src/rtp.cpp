...
RTCPCompoundPacket *RTCPCompoundPacket::Parse( BYTE *data, DWORD size )
{
    ...
    while( bufferLen )
    {
        ...
        {
            //error
            Error( "Wrong rtcp packet type [%d] size %d > %d\n", header->pt, len, bufferLen );
            //Exit
            return NULL;
        }
        //parse
...
RTCPSenderReport::RTCPSenderReport()
    : RTCPPacket( RTCPPacket::SenderReport )
{
    ssrc = 0;
    ntpSec = 0;
    ntpFrac = 0;
...
void RTCPSDES::Description::Dump()
{
    if( items.size() )
    {
        Debug( "\t\t[Description ssrc=0x%X count=%u\n", ssrc, items.size() );
        for( Items::iterator it = items.begin(); it != items.end(); ++it )
            Debug( "\t\t\t[%s '%.*s'/]\n", RTCPSDES::Item::TypeToString( (*it)->GetType() ), (*it)->GetSize(), (*it)->GetData() );
        Debug( "\t\t[/Description]\n" );
    }
    else
        Debug( "\t\t[Description ssrc=0x%X/]\n", ssrc );
}
...
RTPRedundantPacket::RTPRedundantPacket( MediaFrame::Type media, BYTE *data, DWORD size ) : RTPTimedPacket( media, data, size )
{
    ...
    DecodeRedundantPayload( GetMediaData(), GetMediaLength() );
}

RTPRedundantPacket::RTPRedundantPacket( MediaFrame::Type media, DWORD codec ) :
    RTPTimedPacket( media, codec )
{
    primaryCodec = 0;
    primaryType = 0;
    primaryData = NULL;
...
bool RTPRedundantPacket::SetPayload( BYTE *data, DWORD size )
{
    bool ret = RTPPacket::SetPayload( data, size );
    if( ret )
    {
        primaryCodec = 0;
        primaryType = 0;
