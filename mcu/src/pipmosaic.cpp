...
PIPMosaic::PIPMosaic(Mosaic::Type type, DWORD size) : Mosaic(type,size)
{
	...
	//Allocate memory for under image (+20 for allign32)
	underBuffer = (BYTE *) malloc(mosaicSize + FF_INPUT_BUFFER_PADDING_SIZE + 32);
	//Get aligned
...
int PIPMosaic::Update(int pos, BYTE *image, int imgWidth, int imgHeight)
{
	...
	***********************************************/
	...
	DWORD pipIni;
        if (pippos)
        {
            pipIni = SIZE4MUL(intraHeight);
        }
        else
        {
            pipIni = SIZE4MUL(mosaicTotalHeight-mosaicHeight-intraHeight);
        }
	...
	if (pos==0)
	{
		...
		//ApplyParticipantOverlay(pos, imageY, imageU, imageV, imgWidth, imgHeight, true);
		...
		//ApplyParticipantOverlay(pos, lineaY, lineaV ,lineaU, imgWidth, imgHeight, true);
	}
