...
public:
    ...
    RTPRedundantPacket( MediaFrame::Type media, DWORD codec );

    // Override SetPayload to enable decoding on the fly
    bool SetPayload( BYTE *data, DWORD size );
...
private:
    ...
    void DecodeRedundantPayload( BYTE *redpayload, DWORD redsize );
...
public:
    ...
    DWORD GetDelaySinceLastSRMilis()
    {
        ...
        //Return in milis
        return (dslr >> 16) * 1000 + (DWORD)(((double)(dslr & 0xFFFF)) / 65.635);
    }
