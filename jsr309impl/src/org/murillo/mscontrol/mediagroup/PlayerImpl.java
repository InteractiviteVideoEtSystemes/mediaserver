...
public class PlayerImpl implements Player {
	...
	public void play(URI[] movies, RTC[] rtcs, Parameters params) throws MsControlException {
		...
		System.out.println("Méthode Play : #2");
		//Reset repeat count
		repeatCount = 0;
		...
		if (params!=null && params.containsKey(REPEAT_COUNT))
			try {
				...
				System.out.println("Méthode Play : #3");
			} catch (Exception e) {
			}
		//Reset seek
		...
		if (params!=null && params.containsKey(START_OFFSET))
			try {
				...
				System.out.println("Méthode Play : #4");
			} catch (Exception e) {
			}
		...
		System.out.println("Méthode PlayQueue : #11");
	...
	private boolean playQueue(int i) {
		...
		//Get media server
		MediaServer mediaServer = sess.getMediaServer();
		System.out.println("Méthode PlayQueue : #5");
	     group.ConnectPlayer();
		...
		try {
			...
			System.out.println("Méthode PlayQueue : #6");
		} catch (XmlRpcException ex) {
			//Log
		...
		try {
			...
			System.out.println("Méthode PlayQueue : #7");
			}
			...
			else
			{
				...
				System.out.println("Méthode PlayQueue : #8");
		...
		System.out.println("Méthode PlayQueue : #9");
		return true;
	}
	...
	private void fireEventAsync(final PlayerEventImpl event) {
		...
		mediaSession.Exec(new Runnable() {
			@Override
			public void run() {
				System.out.println("Méthode FireEvent : #10");
