...
{
	if (opened) vpx_codec_destroy(&encoder);
	if (pic)
		vpx_img_free(pic);
}
...
int VP8Encoder::SetSize(int width, int height)
{
    //Save values
    if (width <= 0 || height <= 0)
        return Error("VP8Encoder: invalid size %d x %d", width, height);
    
    if (opened)
    {
#ifdef RECONFIGURE_VP8
        // Reconfiguring the codec cause a crash on Omnitor system !
        // Try to reconfigure codec encofding size
	config.g_w = width;
	config.g_h = height;
        ...
        if ( vpx_codec_enc_config_set(&encoder,&config) == VPX_CODEC_OK ) 
        {
            ...
            Log("-VP8Encoder: reconfigured picture size to %d x %d\n",
                width, height);
            return 1;
        }
        else
        {
            return Error("-VP8Encoder: fail to reconfigure picture size to %d x %d\n",
                          width, height);
        }
#else
...
int VP8Encoder::SetFrameRate(int frames,int kbits,int intraPeriod)
{
	...
	if (opened)
	{
		...
		if ( vpx_codec_enc_config_set(&encoder,&config) != VPX_CODEC_OK )
			//Exit
			return Error("-VP8Encoder: fail to reconfigure bitrate and intra-period\n");
...
VideoFrame* VP8Encoder::EncodeFrame(BYTE *buffer,DWORD bufferSize)
{
	...
	{
		Error("-VP8 EncodeFrame length error [%d,%d]\n",numPixels*5/4,bufferSize);
		return NULL;
	}
